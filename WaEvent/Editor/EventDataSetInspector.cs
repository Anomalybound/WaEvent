        ...
        private void DrawEventData(EventData data)
        {
            ...
            if (data.TargetController != null)
            {
                ...
                for (var i = 0; i < animatorController.layers.Length; i++)
                {
                    ...
                    if (i == data.Layer) { GUI.backgroundColor = Color.green; }

                    if (GUILayout.Button(i + " - " + layer.name)) { data.Layer = i; }
                ...
                if (data.Layer != -1)
                {
                    ...
                    var stateMachine = animatorController.layers[data.Layer].stateMachine;
                    for (var i = 0; i < stateMachine.states.Length; i++)
                    {
                        var state = stateMachine.states[i];

                        if (state.state.nameHash == data.StateNameHash) { GUI.backgroundColor = Color.green; }

                        if (GUILayout.Button(state.state.name))
                        {
                            SetupPreview(state.state.motion);
                            previewedMotion = state.state.motion;
                            data.StateNameHash = state.state.nameHash;
                            Debug.Log(data.StateNameHash);
                            _selectedEventArg = null;
                        }
                ...
                if (data.StateNameHash != -1)
                {
                    ...
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        ...
                        if (GUILayout.Button("Add Event", EditorStyles.toolbarButton))
                        {
                            ...
                            {
                                Name = "New Event " + data.Events.Count,
                                NormalizedTime = _normalizedSetting
                            };

                            data.Events.Add(newEventArg);
                            _selectedEventArg = newEventArg;

                            data.Events = data.Events.OrderBy(x => x.NormalizedTime).ToList();
                        }
                    }
                    ...
                    for (var i = 0; i < data.Events.Count; i++)
                    {
                        ...
                        using (new EditorGUILayout.HorizontalScope())
                        {
                            ...
                            if (GUILayout.Button("DEL", GUILayout.Width(45)))
                            {
                                _delayedActions.Add(() => data.Events.Remove(evt));
